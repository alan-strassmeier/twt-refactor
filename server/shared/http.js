...
const parseJsonBody = async (req, maxBytes = 4096) => {
  ...
  if (typeof req.body === 'string') {
    if (Buffer.byteLength(req.body) > maxBytes) {
      throw createHttpError('Payload muito grande.', 413);
    }
  ...
  for await (const chunk of req) {
    size += chunk.length;
    if (size > maxBytes) throw createHttpError('Payload muito grande.', 413);
    chunks.push(chunk);
  }
...
const queryFromRequest = (req) => {
  ...
  if (requestUrl.length > 8192) throw createHttpError('URL de consulta muito grande.', 414);
